...
namespace g2o {
  ...
  EdgeSwitchProx::EdgeSwitchProx()
      : BaseTernaryEdge < 3 * prox_maxN, std::array<int, 2 * prox_maxN>, VertexProx, VertexProx, VertexSwitchingProxPair >(), _num_prox_pairs(0), _prox_pairs(), info(1.) {
  ...
  void EdgeSwitchProx::computeError() {
      ...
      const std::array<number_t, prox_maxN> vk = dynamic_cast<VertexSwitchingProxPair*>(_vertices[2])->estimate();
